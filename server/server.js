...
app.get("/logout", function (req, res) {
  req.session.destroy(err => {
    if (err) {
      // Respond with 500 Internal Server Error if session destruction fails
      console.error("Logout error:", err);
      res.sendStatus(500);
    } else {
      // Respond with 200 OK upon successful logout
      res.sendStatus(200);
    }
  });
});
    ...
    fetch(url, { signal: controller.signal })
      .then(apiRes => {
        ...
        return apiRes.text().then(data => {
          ...
          // Check if OMDb returned a valid movie response
          if (response.Response === 'True') {
            // Convert OMDb response format to our internal movie format
            const movieData = {
            ...
            // If OMDb couldn't find the movie, return 404
            res.sendStatus(404);
          }
        });
      })
